...
Write a function to query the API (use the `GET()` function from `library(httr)`). A helper function is needed to take what is returned by `get` to turn it into a "nice `tibble`".
